...
namespace MavLink
{
    ...
    public static class MavLinkSerializer
    {
        ...
        public static Dictionary<int, MavPacketInfo> Lookup = new Dictionary<int, MavPacketInfo>
        {
			...
			{195, new MavPacketInfo(Deserialize_DEEPSTALL, 120)},
		...
		internal static MavlinkMessage Deserialize_DEEPSTALL(byte[] bytes, int offset)
		{
			return new Msg_deepstall
			{
				landing_lat = bitconverter.ToInt32(bytes, offset + 0),
				landing_lon = bitconverter.ToInt32(bytes, offset + 4),
				path_lat = bitconverter.ToInt32(bytes, offset + 8),
				path_lon = bitconverter.ToInt32(bytes, offset + 12),
				arc_entry_lat = bitconverter.ToInt32(bytes, offset + 16),
				arc_entry_lon = bitconverter.ToInt32(bytes, offset + 20),
				altitude = bitconverter.ToSingle(bytes, offset + 24),
				expected_travel_distance = bitconverter.ToSingle(bytes, offset + 28),
				cross_track_error = bitconverter.ToSingle(bytes, offset + 32),
				stage = bytes[offset + 36],
			};
		}
		...
		internal static int Serialize_DEEPSTALL(this Msg_deepstall msg, byte[] bytes, ref int offset)
		{
			bitconverter.GetBytes(msg.landing_lat, bytes, offset + 0);
			bitconverter.GetBytes(msg.landing_lon, bytes, offset + 4);
			bitconverter.GetBytes(msg.path_lat, bytes, offset + 8);
			bitconverter.GetBytes(msg.path_lon, bytes, offset + 12);
			bitconverter.GetBytes(msg.arc_entry_lat, bytes, offset + 16);
			bitconverter.GetBytes(msg.arc_entry_lon, bytes, offset + 20);
			bitconverter.GetBytes(msg.altitude, bytes, offset + 24);
			bitconverter.GetBytes(msg.expected_travel_distance, bytes, offset + 28);
			bitconverter.GetBytes(msg.cross_track_error, bytes, offset + 32);
			bytes[offset + 36] = msg.stage;
			offset += 37;
			return 195;
		}
